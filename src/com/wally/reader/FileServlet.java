...
public class FileServlet extends HttpServlet 
{
	...
	private static String getUpdatePath(String version,String model,HttpServletRequest request, HttpServletResponse response)
	{
		...
		else
		{
			...
			String srcFileName = childFiles[0].getName();
			srcFileName = srcFileName.substring(0,srcFileName.length()-4);
			
			int apkNum =  Integer.parseInt(version);
			int srcNum =  Integer.parseInt(srcFileName);
			if(srcNum > apkNum)
			{
				path = getProjectPath(request, response) + "srcfile/update/" + childFiles[0].getName();
			}
		}
		return path;
	}
		...
		else
		{
			...
			for (File child : childFiles) 
			{
				...
				//	entity.setName(child.getName());
					...
					String path = getProjectPath(request, response) + "srcfile/content/";
