...
public class AdventureServlet extends HttpServlet {
	...
	private static final long serialVersionUID = -5176222149890540022L;
	
	private static String FILE_ADD = "D:/bin/apache-tomcat-6.0.35/webapps/dbtest/tintin/content";
	//D:/bin/apache-tomcat-6.0.35/webapps/Tintin/tintin
	private static String APP_ADD = "D:/bin/apache-tomcat-6.0.35/webapps/dbtest/tintin/update";
	//"D:/bin/apache-tomcat-6.0.35/webapps/Tintin/tintin/update"
	...
	private static String getUpdatePath(String version,String model,HttpServletRequest request, HttpServletResponse response)
	{
		...
		else
		{
			...
			String tintinName = childFiles[0].getName();
			tintinName = tintinName.substring(0,tintinName.length()-4);
			
			int apkNum =  Integer.parseInt(version);
			int srcNum =  Integer.parseInt(tintinName);
			if(srcNum > apkNum)
			{
				path = getProjectPath(request, response) + "tintin/update/" + childFiles[0].getName();
			}
		}
		return path;
	}
		...
		else
		{
			...
			for (File child : childFiles) 
			{
				...
				//	entity.setName(child.getName());
					...
					String path = getProjectPath(request, response) + "tintin/content/";
